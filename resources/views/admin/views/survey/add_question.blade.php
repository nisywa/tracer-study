...
<form id="surveyForm" action="{{ route('admin.survey.create_question') }}" method="POST">
                                        ...
                                        <tr>
                                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                <div class="flex flex-col px-2 py-1">
                                                    <input type="text" name="pertanyaan[]" value="{{ $question->pertanyaan }}" class="text-sm leading-normal dark:text-white bg-transparent outline-none question-input" placeholder="Tulis Pertanyaan disini">
                                                </div>
                                            </td>
                                            <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                <div class="flex flex-col px-2 py-1">
                                                    <input type="text" name="deskripsi[]" value="{{ $question->deskripsi }}" class="text-sm leading-normal dark:text-white bg-transparent outline-none description-input" placeholder="Tulis Deskripsi disini">
                                                </div>
                                            </td>
                                            <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                <div class="flex flex-col px-2 py-1">
                                                    <input type="text" name="blok[]" value="{{ $question->blok }}" class="text-sm leading-normal dark:text-white bg-transparent outline-none question-input" placeholder="Blok Pertanyaan">
                                                </div>
                                            </td>
                                            <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <select name="tipe[]" class="text-xs font-semibold leading-tight border border-gray-400 rounded px-2 py-1 input-type" onchange="changeInputType(this)">
                                                    <option value="">Pilih Tipe</option>
                                                    <option value="text" {{ $question->tipe == 'text' ? 'selected' : '' }}>Short Answer</option>
                                                    <option value="textarea" {{ $question->tipe == 'textarea' ? 'selected' : '' }}>Paragraph</option>
                                                    <option value="checkbox" {{ $question->tipe == 'checkbox' ? 'selected' : '' }}>Checkboxes</option>
                                                    <option value="radio" {{ $question->tipe == 'radio' ? 'selected' : '' }}>Radio</option>
                                                    <option value="select" {{ $question->tipe == 'select' ? 'selected' : '' }}>Dropdown</option>
                                                    <option value="file" {{ $question->tipe == 'file' ? 'selected' : '' }}>File</option>
                                                    <option value="date" {{ $question->tipe == 'date' ? 'selected' : '' }}>Datepicker</option>
                                                </select>
                                            </td>
                                            <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent personal-column">
                                                    ...
                                                    <div id="optionContainer" class="option-container">
                                                        @foreach($question->template_jawaban as $option)
                                                            <div class="option-item flex items-center space-x-2 mb-2">
                                                                <input type="text" name="options[{{ $question->id }}][]" value="{{ $option->pilihan_jawaban }}" class="text-xs border border-gray-400 rounded px-2 py-1 flex-grow" />
                                                                <div class="flex space-x-1">
                                                                    <button type="button" class="move-option-up text-gray-500 hover:text-gray-700">
                                                                        <i class="fas fa-arrow-up"></i>
                                                                    </button>
                                                                    <button type="button" class="move-option-down text-gray-500 hover:text-gray-700">
        ...
        <div class="flex justify-end p-4">
            <button type="submit" class="px-8 py-2 font-bold text-white bg-green-500 rounded-lg shadow-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50">
     ...
     <table style="display: none;">
            <tbody>
                <tr id="templateRow">
                    <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <div class="flex flex-col px-2 py-1">
                            <input type="text" name="pertanyaan[]" class="text-sm leading-normal dark:text-white bg-transparent outline-none question-input" placeholder="Tulis Pertanyaan disini">
                        </div>
                    </td>
                    <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <div class="flex flex-col px-2 py-1">
                            <input type="text" name="deskripsi[]" class="text-sm leading-normal dark:text-white bg-transparent outline-none description-input" placeholder="Tulis Deskripsi disini">
                        </div>
                    </td>
                    <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <div class="flex flex-col px-2 py-1">
                            <input type="text" name="blok[]" class="text-sm leading-normal dark:text-white bg-transparent outline-none question-input" placeholder="Blok Pertanyaan">
                        </div>
                    </td>
                    <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                      <select name="tipe[]"
